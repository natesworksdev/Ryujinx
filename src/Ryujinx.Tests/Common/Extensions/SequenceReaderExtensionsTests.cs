...
{
    class SequenceReaderExtensionsTests
    {
        [TestCase(null)]
        [TestCase(sizeof(int) + 1)]
        public unsafe void GetRefOrRefToCopy_ReadsMultiSegmentedSequenceSuccessfully(int? maxSegmentSize)
        {
            // Arrange
            MyUnmanagedStruct[] originalStructs = EnumerateNewUnmanagedStructs().Take(3).ToArray();

            ReadOnlySequence<byte> sequence = CreateSegmentedByteSequence(originalStructs, maxSegmentSize ?? Unsafe.SizeOf<MyUnmanagedStruct>());
        ...
        [Test]
        public unsafe void GetRefOrRefToCopy_FragmentedSequenceReturnsRefToCopy()
        {
            // Arrange
        ...
        [Test]
        public unsafe void GetRefOrRefToCopy_ContiguousSequenceReturnsRefToBuffer()
        {
            // Arrange
            MyUnmanagedStruct[] originalStructs = EnumerateNewUnmanagedStructs().Take(1).ToArray();
        ...
        [Test]
        public unsafe void GetRefOrRefToCopy_ThrowsWhenNotEnoughData()
        {
            // Arrange
            MyUnmanagedStruct[] originalStructs = EnumerateNewUnmanagedStructs().Take(1).ToArray();
