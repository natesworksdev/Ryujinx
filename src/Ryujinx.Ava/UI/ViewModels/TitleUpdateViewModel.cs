    ...
    public class TitleUpdateViewModel : BaseModel
    {
        ...
        private void AddUpdate(string path)
        {
            if (File.Exists(path) && TitleUpdates.All(x => x.Path != path))
            {
                ...
                {
                    Dispatcher.UIThread.InvokeAsync(() => ContentDialogHelper.CreateErrorDialog(LocaleManager.Instance.UpdateAndGetDynamicValue(LocaleKeys.DialogLoadErrorMessage, ex.Message, path)));
                }
            }
        }
