...
namespace Ryujinx.Ui
{
    public class GameTableContextMenu : Menu
    {
            ...
            if (response == (int)ResponseType.Accept)
            {
                Thread extractorThread = new Thread(() =>
                {
                    ...
                    using (FileStream file = new FileStream(sourceFile, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        string titleUpdateMetadataPath = System.IO.Path.Combine(AppDataManager.GetGamesPath(), mainNca.Header.TitleId.ToString("x16"), "updates.json");
        ...
        {
            string titleId = _gameTableStore.GetValue(_rowIter, 2).ToString().Split("\n")[1].ToLower();
            string ptcDir  = System.IO.Path.Combine(AppDataManager.GetGamesPath(), titleId, "cache", "cpu");
        ...
        private void PurgePtcCache_Clicked(object sender, EventArgs args)
        {
            ...
            DirectoryInfo mainDir   = new DirectoryInfo(System.IO.Path.Combine(AppDataManager.GetGamesPath(), titleId, "cache", "cpu", "0"));
            DirectoryInfo backupDir = new DirectoryInfo(System.IO.Path.Combine(AppDataManager.GetGamesPath(), titleId, "cache", "cpu", "1"));
