...
namespace Ryujinx.Tests.Cpu
{
    ...
    public class CpuTest
    {
        ...
        public void Setup()
        {
            ...
            _memory = new AMemory(_ramPointer);
            _memory.Map(Position, 0, _size);
            _thread = new AThread(translator, _memory, _entryPoint);
        ...
        private void ManageFpTolerances(FpTolerances fpTolerances)
        {
            ...
            bool IsNormalOrSubnormalS(float f)
            {
                return float.IsNormal(f) || float.IsSubnormal(f);
            }

            bool IsNormalOrSubnormalD(double d)
            {
                return double.IsNormal(d) || double.IsSubnormal(d);
            }
        }
