...
namespace Ryujinx.Graphics.GAL.Multithreading.Commands.Window
{
    struct WindowPresentCommand : IGALCommand
    {
        ...
        private TableRef<Action<int>> _swapBuffersCallback;

        public void Set(TableRef<ThreadedTexture> texture, ImageCrop crop, TableRef<Action<int>> swapBuffersCallback)
        {
            _texture = texture;
            _crop = crop;
