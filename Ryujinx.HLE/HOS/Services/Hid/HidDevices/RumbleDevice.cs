    ...
    public class RumbleDevice
    {
        ...
        public void RumbleMultiple(ReadOnlySpan<HidVibrationValue> values)
        {
            for (int i = 0; i < values.Length; i++)
            {
                ...
                {
                    if (GamePad.GetName(_gamepadIndex).Contains("xinput", StringComparison.OrdinalIgnoreCase))
                    {
                        // Unsafe access to internal XInput joystick for proper vibration
                        Type XInputJoystick = typeof(OpenTK.BezierCurve).Assembly.GetType("OpenTK.Platform.Windows.XInputJoystick");
                        ...
                        XInputJoystick.GetMethod("SetVibration").Invoke(joystick, new object[] {
                            _gamepadIndex, value.AmplitudeLow, value.AmplitudeHigh
                        });
                    } else
                    {
                        // DInput is not supported
