...
namespace Ryujinx.HLE.HOS.Services.Mii.StaticService
{
    abstract class IDatabaseService : IpcService
    {
        ...
        private Span<byte> CreateByteSpanFromBuffer(ServiceCtx context, in IpcBuffDesc ipcBuff, bool isOutput)
        {
            byte[] rawData;
        ...
        private Span<T> CreateSpanFromBuffer<T>(ServiceCtx context, in IpcBuffDesc ipcBuff, bool isOutput) where T: unmanaged
        {
            return MemoryMarshal.Cast<byte, T>(CreateByteSpanFromBuffer(context, ipcBuff, isOutput));
        }

        private void WriteSpanToBuffer<T>(ServiceCtx context, in IpcBuffDesc ipcBuff, Span<T> span) where T: unmanaged
        {
