...
namespace Ryujinx.HLE.HOS
{
    public class TamperMachine
    {
        ...
        private ConcurrentQueue<ITamperProgram> _programs = new();
        private long _pressedKeys = 0;
        private Dictionary<string, ITamperProgram> _programDictionary = new();

        private void Activate()
        {
            if (_tamperThread == null || !_tamperThread.IsAlive)
            {
                _tamperThread = new Thread(TamperRunner)
                {
