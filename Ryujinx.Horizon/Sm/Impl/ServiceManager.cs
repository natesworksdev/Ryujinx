...
namespace Ryujinx.Horizon.Sm.Impl
{
    class ServiceManager
    {
        ...
        public Result GetService(out int handle, long processId, ServiceName name)
        {
            handle = 0;
        ...
        {
            return KernelStatic.Syscall.ConnectToPort(serviceInfo.PortHandle, out handle);
        }

        public Result RegisterService(out int handle, long processId, ServiceName name, int maxSessions, bool isLight)
        {
            handle = 0;
            Result result = ValidateServiceName(name);
        ...
        private Result RegisterServiceImpl(out int handle, long processId, ServiceName name, int maxSessions, bool isLight)
        {
            handle = 0;
            ...
            result = KernelStatic.Syscall.CreatePort(maxSessions, isLight, 0, out handle, out int clientPort);
        ...
        public Result UnregisterService(long processId, ServiceName name)
        {
            Result result = ValidateServiceName(name);
