...
namespace Ryujinx.Horizon.Sdk.Sf.Hipc
{
    class ServerSessionManager
    {
        ...
        private Result AcceptSessionImpl(out ServerSession session, int portHandle, ServiceObjectHolder obj)
        {
            ...
            Result result = KernelStatic.Syscall.AcceptSession(portHandle, out int sessionHandle);
            ...
            finally
            {
                if (!succeeded)
                {
                    KernelStatic.Syscall.CloseHandle(sessionHandle);
                }
            }
        ...
        {
            session = new ServerSession(sessionHandle, obj);

            // TODO: Set pointer buffer and saved message buffer.
        ...
        private void CloseSessionImpl(ServerSession session)
        {
            int sessionHandle = session.Handle;
            Os.FinalizeWaitableHolder(session);
            DestroySession(session);
            KernelStatic.Syscall.CloseHandle(sessionHandle).AbortOnFailure();
        }
        ...
        private Result ReceiveRequestImpl(ServerSession session, Span<byte> message)
        {
            while (true)
            {
                if (!session.PointerBuffer.IsEmpty)
                {
            ...
            Span<byte> outMessage)
        {
            ...
            {
                KernelStatic.Syscall.CloseHandle(handlesToClose[i]).AbortOnFailure();
            }
